...
import 'package:lane_dane/utils/create_amount_image_util.dart';
import 'package:lane_dane/utils/init.dart';
import 'package:lane_dane/utils/save_util.dart';
...
class NotificationService {
  ...
  Future<void> showIncomingTransactionSmsNotification({
    required PersonalTransaction personalTransaction,
  }) async {
    img.Image image = await createBlankFilledImage(
      backgroundColor: const Color(0xFFFAFAD2),
    );
    ...
    image = await addTitleToImage(
      image: image,
      text: personalTransaction.transactionType.toLowerCase() == 'credit'
          ? 'Credited'
          : 'Debited',
      textColor: const Color(0xFF6F8FAF),
    );
    ...
    image = await addMainContentToImage(
      image: image,
      text: 'Rs. ${personalTransaction.amount}',
      textColor: const Color(0xFF6495ED),
    );

    image = await addTransactionTypeIcon(
      type: personalTransaction.transactionType.toLowerCase() == 'debit'
    ...
    if (locale.languageCode == 'hi') {
      String type =
          personalTransaction.transactionType.toLowerCase() == 'credit'
              ? 'received'
              : 'spent';
      notificationMessage =
          'You have just $type ₹ ${personalTransaction.amount}';
    } else {
      String type =
          personalTransaction.transactionType.toLowerCase() == 'credit'
              ? 'received'
    ...
    String payload = personalTransaction.toJson();
