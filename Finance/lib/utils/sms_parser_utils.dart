...
import 'package:lane_dane/utils/extensions/sms_message_extension.dart';
...
// --------------------------
PersonalTransaction? parseSmsToTransaction(SmsMessage message) {
  // if (!isInbox(message)) {
  //   return null;
  // }
  ...
  return PersonalTransaction(
      ...
      accNumber: parsedAccountNumber,
      smsBody: messageBody,
      transactionType: parsedTransactionType,
      createdAt: datetime,
      updatedAt: datetime,
      description: messageBody ?? 'SMS');
}
...
int? parseAmount(SmsMessage message) {
  ...
  final RegExp reg = RegExp(Constants.kRegexpAmount);
...
String? parseAccountNumber(SmsMessage message) {
  ...
  final RegExp reg =
      RegExp(Constants.kRegexpAccountNumber, caseSensitive: false);
